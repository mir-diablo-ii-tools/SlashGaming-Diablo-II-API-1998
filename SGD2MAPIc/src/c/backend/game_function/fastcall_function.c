...
ASM_X86_LABEL(CallFastcallFunction_PushStackArgsLoopEnd);
  ...
  /*
  * We do not conditionally branch when setting the ecx or edx
  * registers, because it incurs additional cost (in readability and
  * execution) with no realistic benefit. the fact that if the called
  * function has no parameters, the register values are overriden
  * anyways.
  */
